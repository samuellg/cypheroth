...
declare -a queries=(
    "All Objects that are members of High Value Groups;MATCH p=(n)-[r:MemberOf*1..]->(m:Group {highvalue:true}) SET n.highvalue=true RETURN labels(n) AS ObjectType,n.name AS ObjectName,m.name AS HighValueGroupName ORDER BY ObjectType ASC;AllHighValueObjects.csv"
